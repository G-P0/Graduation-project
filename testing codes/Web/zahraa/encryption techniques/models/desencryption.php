...
//parity drop & initial permutation of key (56 bit key)(minus 1 due to index starting from 0)
...
//////////////////////////////////////////////////////////////////////////////////////////
		...
		for ($i=1; $i <= 16; $i++) {
			...
			//print_r($temp_arr);
			//xor the output of D box with key
			...
			for ($j=1; $j <= 8; $j++) {
				...
				$m=bindec($B[$j][0].$B[$j][5]);//first and last bit with each other converted to decimal
				$n=bindec($B[$j][1].$B[$j][2].$B[$j][3].$B[$j][4]);//in between 4 bits with each other
				//echo $B[$j][0].$B[$j][5]."*";
				//echo $B[$j][1].$B[$j][2].$B[$j][3].$B[$j][4]."|";
				
				$temp=decbin($S[$j][$m][$n]);//bring appropriate value from appropriate s box and covert it to bin
				//echo($temp."|");
				//fill with zeros	
			    $temp=str_pad($temp, 4, '0', STR_PAD_LEFT);//to make binary result represented in 4 bits
				$B[$j]=$temp;
				//print_r($B[$j]."\n");
			}	
			...
			//swapper
			//first prev right in current left
			$L[$i] = $R[$i-1];
			//o/p of des func on right ($temp_arr2) xored with prev left in current right 
			for ($k=0; $k < 32; $k++) { //loop on 32 bit of two parts and xor them
				//echo($L[$i-1][$k]);
	...
	$PRet[17-1],$PRet[57-1],$PRet[25-1]);
		...
		echo "<br><b> Encrypted Message : " . $ciphertext . "</b>"; 

//$key =     "0001001100110100010101110111100110011011101111001101111111110001";
//$message = "0000000100100011010001010110011110001001101010111100110111101111";
//decryptedMsg = "0000101001001100110110011001010101000011010000100011001000110100";
?>